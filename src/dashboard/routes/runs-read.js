...
const REPORT_FILE_BASENAMES = new Set(['new-urls.csv', 'new-urls.json', 'new-games.csv', 'unknown-urls.csv', 'report.md']);
const CONTENT_TYPES = {
 ...
 * 只读分页：本里程碑只有"新增"这一种变化类型（missing/removed/restored
 * 属于 M4，明确不在本阶段实现），type 只接受 'added' 或不传。
 */
export function getRunChangesRoute(db, runController, runId, { siteId, type, page, pageSize } = {}) {
  assertRunKnown(runController, runId);
  if (type !== undefined && type !== 'added') {
    throw new ApiError('INVALID_PAGE_PARAMS', `本里程碑只支持 type=added，收到: ${type}`, { status: 422 });
  }
